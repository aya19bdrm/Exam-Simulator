import type { ExamReport, ThemedStyles } from '../../../types'

import React from 'react'
import styled from 'styled-components'
import { darken } from 'polished'
import { formatDate, formatTimer } from '../../../utils/format'
import { analyzeReviewGridItem } from '../../../utils/analyze'
...
export default ({ report }: ReviewSummaryProps): React.JSX.Element => (
  <SummaryStyles status={report.status}>
    ...
    <Chart items={report.testLength}>{renderChartBars(report)}</Chart>
  </SummaryStyles>
)

function renderChartBars(report: ExamReport): React.JSX.Element[] {
  return report.intervals.map((interval, i) => {
    const max = Math.max(...report.intervals)
    const ratio = 150 / max

    return (
      <div key={i} className="row">
        <ChartBar width={interval * ratio} background={analyzeReviewGridItem(i, report)}>
          <div className="tooltip">{formatTimer(interval)}</div>
        </ChartBar>
...
export interface ChartProps extends ThemedStyles {
  items: number
}

export interface ChartBarProps extends ThemedStyles {
  width: number
  /** color */
  background: string
}
